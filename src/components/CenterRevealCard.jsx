...
function useAudio() {
  ...
  const playDot = async () => { // оставляем как отдельный тембр для соц-иконок
    const ctx = await getCtx(); if (!ctx) return;
    const t0 = ctx.currentTime;
...
/* ===== DESKTOP Card (без кружков; SHOWREEL кликабелен) ===== */
function DesktopCard() {
  ...
  const openShowreel = () => {
    setVimeoId("1001147905"); // прежний ролик из кружка №3
    setPlayerOpen(true);
  };
